...
const parseModel=(model,erd,models,relations,options,fake=false)=>{
  ...
  for (let key in paths) {
    ...
    if (paths[key].options.ref) {
      ...
      const collection2Fields = Object.keys(models[collection2Name][`${models[collection2Name]['modelName']}Schema`].paths);
...
const generateFromModels = async (modelsArray, options = {}) => {
  try {
    ...
    for (const model of modelsArray) {
      if(model[`${model.modelName}Schema`]){
        console.log(`${model.modelName}Schema`)
        models[model.modelName] = model;
        modelNameToCollectionName[model.modelName] = model.modelName;
      }
